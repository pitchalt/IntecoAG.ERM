...
namespace IntecoAG.ERM.CRM.Contract
{
    ...
    public class DealWithoutStageTests : DealBaseTest
    {
        ...
        private crmContractRegistrationForm NewRegistrationFormPrepare(Session ssn) {
            ...
            frm.OurParty = ssn.GetObjectByKey<crmPartyRu>(partyRu1.Oid);
            frm.PartnerParty = ssn.GetObjectByKey<crmPartyRu>(partyRu2.Oid);
        ...
        public void CreateAllReferencesOnlyTest([Values("", "0", "1", "2", "3", "4")]  string modificator) {
            ...
            crmPhysicalPersonRu physicalPersonRu = Prepare_crmPhysicalPersonRu(ssn, modificator, address);
            if (Common.CheckValidationRule) {
                ruleResult = ruleSet.ValidateTarget(physicalPersonRu, DefaultContexts.Save);
                Assert.AreEqual(ValidationState.Valid, ruleResult.State);
            }

            crmBusinessmanRu businessmanRu = Prepare_crmBusinessmanRu(ssn, modificator, physicalPersonRu, address);
            if (Common.CheckValidationRule) {
                ruleResult = ruleSet.ValidateTarget(businessmanRu, DefaultContexts.Save);
                Assert.AreEqual(ValidationState.Valid, ruleResult.State);
            }

            crmLegalPersonRussianRu legalPersonRussianRu = Prepare_crmLegalPersonRussianRu(ssn, modificator, address);
            if (Common.CheckValidationRule) {
            ...
            SubjectClass subject = Prepare_fmSubject(ssn, modificator);
            if (Common.CheckValidationRule) {
            ...
            crmUserParty userParty = Prepare_crmUserParty(ssn, modificator, partyRu);
            if (Common.CheckValidationRule) {
