...
namespace IntecoAG.ERM.FM.PaymentRequest {
    [NonPersistent]
    public class fmCPRPaymentRequestFilter01 : ReportParametersObjectBase, ICustomFilter {
        public fmCPRPaymentRequestFilter01(Session session) : base(session) { }
        ...
        private CriteriaOperator SearchCriteriaObjectBuilder() {
            ...
            if (this.DateInvoiceBegin != default(DateTime)) {
                criteria.Operands.Add(new BinaryOperator("ExtDocDate", this.DateInvoiceBegin.Date, BinaryOperatorType.GreaterOrEqual));
            }
            if (this.DateInvoiceEnd != default(DateTime)) {
                criteria.Operands.Add(new BinaryOperator("ExtDocDate", this.DateInvoiceEnd.Date.AddDays(1), BinaryOperatorType.Less));
            }
            ...
            if (!String.IsNullOrEmpty(this.InvoiceNumber)) {
                criteria.Operands.Add(new BinaryOperator("ExtDocNumber", this.InvoiceNumber, BinaryOperatorType.Like));
            }
