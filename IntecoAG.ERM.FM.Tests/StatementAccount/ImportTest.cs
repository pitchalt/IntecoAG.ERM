...
namespace IntecoAG.ERM.FM.Tests.StatementAccount {
    class ImportTest : BaseTest {
        ...
        [Test]
        public void TestImportVTB() {
            IObjectSpace os;
            using (os = Application.CreateObjectSpace()) {
                //
                fmCSAImportResult result = ImportVTB(os);
                os.CommitChanges();
                Assert.AreEqual(result.ResultCode, 1);
                Assert.AreEqual(result.StatementOfAccounts.Count, 5);
            }
        }
        ...
        [Test]
        public void TestImportResultProcessActionVTB() {
            IObjectSpace os;
            using (os = Application.CreateObjectSpace()) {
                fmCSAImportResult result = ImportVTB(os);
                os.CommitChanges();
        ...
        [Test]
        public void TestImportResultProcessActionSBRF() {
            IObjectSpace os;
            using (os = Application.CreateObjectSpace()) {
                fmCSAImportResult result = ImportSBRF(os);
                os.CommitChanges();
