...
namespace IntecoAG.ERM.CRM.Contract.Deal
{
    ...
    public partial class crmDealWithoutStage : crmContractDeal, IVersionBusinessLogicSupport, IVersionMainObject
    {
        ...
        private void AddPFRegisterRecords(crmDealWithoutStageVersion scVersion) {
            ...
            using (UnitOfWork uow = this.Session.BeginNestedUnitOfWork()) {
                ...
                /* // Старьё закомментарено:
                if (scVersion1.DeliveryUnit != null) {
                    foreach (crmDeliveryItem di in scVersion1.DeliveryUnit.DeliveryItems) {
                        //pfrColl.Add(CreatePlaneFactRegister(scVersion, di));
                        CreatePlaneFactRegister(uow, scVersion1, di);
                    }
                }

                // Пишем записи для каждой поставки
                if (scVersion1.DeliveryUnits != null) {
                    foreach (crmDeliveryUnit du in scVersion1.DeliveryUnits) {
                ...
                */
                ...
                //foreach (crmDeliveryPlan dp in scVersion1.DeliveryPlans) {
                //    foreach (crmDeliveryUnit du in dp.DeliveryUnits) {
                //        foreach (crmDeliveryItem di in du.DeliveryItems) {
                //            //pfrColl.Add(CreatePlaneFactRegister(scVersion, di));
                //            CreatePlaneFactRegister(scVersion1, di);
                //        }
                //    }
                //}

                //pfrColl.Session.Save(pfrColl);
                uow.CommitChanges();
            }
        ...
        private void AddDCDRegisterRecords(crmDealWithoutStageVersion scVersion) {
            ...
            using (UnitOfWork uow = this.Session.BeginNestedUnitOfWork()) {
                ...
                // Заполняем две записи. Вотрая является инвертированной к первой по отношению Первичная сторона - Вторичная сторона
                ...
                //dcdr1.Subject = scVersion1.Order.Subject;
                ...
                //dcdr2.Subject = scVersion1.Order.Subject;
        ...
        private void AddCFRegisterRecords(crmDealWithoutStageVersion scVersion) {
            ...
            using (UnitOfWork uow = this.Session.BeginNestedUnitOfWork()) {
                ...
                //// Пишем записи для каждой поставки
                //if (scVersion1.Settlement != null) {
                //    foreach (crmPaymentItem pi in scVersion1.Settlement.PaymentItems) {
                ...
                //foreach (crmPaymentPlan pp in scVersion.PaymentPlans) {
                //    foreach (crmPaymentUnit du in pp.PaymentUnits) {
                //        foreach (crmPaymentItem di in du.PaymentItems) {
                //            cfrColl.Add(CreateCFRegister(scVersion, di));
                //        }
                //    }
                //}

                //cfrColl.Session.Save(cfrColl);
                uow.CommitChanges();
            }
        }
