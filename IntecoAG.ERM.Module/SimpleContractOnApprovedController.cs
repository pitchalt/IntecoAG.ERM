...
namespace IntecoAG.ERM.Module {
    public partial class SimpleContractOnApprovedController : ViewController<DetailView> {
        //public LoadTaskOnCreateController() {
        //    InitializeComponent();
        //    RegisterActions(components);
        //}
        public SimpleContractOnApprovedController() {
        ...
        protected override void OnActivated() {
            ...
            foreach (DevExpress.ExpressApp.Actions.ActionBase action in Frame.Controllers[typeof(DevExpress.ExpressApp.SystemModule.ObjectMethodActionsViewController)].Actions) {
                switch (action.Id) {
                    case "SimpleContractVersion.Approve":
                        action.Executed += new EventHandler<ActionBaseEventArgs>(Approve_Executed);
                        break;
        ...
        void Approve_Executed(object sender, ActionBaseEventArgs e) {
            // Загружаем новый Task

            View view = View;

            IObjectSpace objectSpace = (IObjectSpace)view.ObjectSpace;

            ((ObjectSpace)objectSpace).CommitChanges();

            if (View.CanClose()) View.Close();

            /*
            TaskInvoiceInstanceDefinitionAdmin currentObject = (TaskInvoiceInstanceDefinitionAdmin)View.CurrentObject;
            currentObject.Save();
