        ...
        */
        ...
        private CriteriaOperator SearchCriteriaObjectBuilder() {
            ...
            if (this.DateRegistrationEnd != System.DateTime.MinValue) {
                CriteriaOperator criteriathisDateRegistrationEnd = new BinaryOperator("DateRegistration", this.DateRegistrationEnd, BinaryOperatorType.LessOrEqual);
            ...
            CriteriaOperator criteriaContractKind = null;
            if (this.ContractKind != 0) {
                criteriaContractKind = new BinaryOperator("ContractKind", ContractKind, BinaryOperatorType.Equal);
                ((GroupOperator)criteriaAND).Operands.Add(criteriaContractKind);
            }
            ...
            if (this.Valuta != null) {
                criteriaValuta = new BinaryOperator("Current.Valuta.Oid", Valuta.Oid, BinaryOperatorType.Equal);
                ((GroupOperator)criteriaAND).Operands.Add(criteriaValuta);
            }

            if (this.PriceFrom != 0) {
                CriteriaOperator criteriathisPriceFrom = new BinaryOperator("Current.Price", this.PriceFrom, BinaryOperatorType.GreaterOrEqual);
                ((GroupOperator)criteriaAND).Operands.Add(criteriathisPriceFrom);
            }

            if (this.PriceTo != 0) {
                CriteriaOperator criteriathisPriceTo = new BinaryOperator("Current.Price", this.PriceTo, BinaryOperatorType.LessOrEqual);
            ...
            //    CriteriaOperator criteriaDocumentNumberILike = null;
            //    if (!string.IsNullOrEmpty(this.DocumentNumber)) {
            //        criteriaDocumentNumberILike = CriteriaOperator.Parse(string.Format("IList({0}, {1})", "ContractDocument.Number", "'%" + this.DocumentNumber + "%'"));
            //        ((GroupOperator)criteriaOR).Operands.Add(criteriaDocumentNumberILike);
            //    }

            //    CriteriaOperator criteriaDescriptionShortILike = null;
            //    if (!string.IsNullOrEmpty(this.Description)) {
            //        //criteriaDescriptionShortILike = CriteriaOperator.Parse(string.Format("IList({0}, {1})", "Current.DescriptionShort", "'%" + this.Description + "%'"));
            //        //criteriaDescriptionShortILike = CriteriaOperator.Parse(string.Format("IList({0}, {1})", "Current.DescriptionShort", "'" + this.Description + "'"));
            //        criteriaDescriptionShortILike = CriteriaOperator.Parse("IList(Current.DescriptionShort, ?)", "%" + this.Description + "%");
            //        ((GroupOperator)criteriaOR).Operands.Add(criteriaDescriptionShortILike);
            //    }
            //}
