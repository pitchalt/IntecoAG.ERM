...
namespace IntecoAG.ERM.FM.StatementAccount {
    ...
    [NonPersistent]
    public class fmCSAImportResultFilter01 : ReportParametersObjectBase, ICustomFilter {

        public fmCSAImportResultFilter01(Session session) : base(session) {
        }
        ...
        [Custom("Caption", "Дата загрузки с")]
        ...
        [Custom("Caption", "Дата загрузки по")]
        ...
        // BEGIN
        protected DateTime _DateAccountBegin;
        [Custom("Caption", "Начальная дата выписки позже, чем")]
        public DateTime DateAccountBegin {
            get {
                return _DateAccountBegin;
            }
            set {
                SetPropertyValue<DateTime>("DateAccountBegin", ref _DateAccountBegin, value);
            }
        }

        protected DateTime _DateAccountEnd;
        [Custom("Caption", "Конечная дата выписка ранее, чем")]
        public DateTime DateAccountEnd {
            get {
                return _DateAccountEnd;
            }
            set {
                SetPropertyValue<DateTime>("DateAccountEnd", ref _DateAccountEnd, value);
            }
        }
        ...
        private CriteriaOperator SearchCriteriaObjectBuilder() {
            ...
            if (this.DateBegin != System.DateTime.MinValue) {
                criteria.Operands.Add(new BinaryOperator("ImportDate", this.DateBegin.Date, BinaryOperatorType.GreaterOrEqual));
            }
            if (this.DateEnd != System.DateTime.MinValue) {
                criteria.Operands.Add(new BinaryOperator("ImportDate", this.DateEnd.Date.AddDays(1), BinaryOperatorType.Less));
            }
            if (this.DateAccountBegin != System.DateTime.MinValue) {
                criteria.Operands.Add(new BinaryOperator("DateFrom", this.DateAccountBegin.Date, BinaryOperatorType.GreaterOrEqual));
            }
            if (this.DateAccountEnd != System.DateTime.MinValue) {
                criteria.Operands.Add(new BinaryOperator("DateTo", this.DateAccountEnd.Date.AddDays(1), BinaryOperatorType.Less));
            }
            if (this.Bank != null) {
